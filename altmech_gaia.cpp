...
vector < vector <int> > nutrient_genome_interactions(int genome_length, int num_nutrients, default_random_engine &generator){
    ...
    for (int i = 1; i < pow(2,genome_length); i++){
      int pos = 0;
      int neg = 0;
...
vector < double > abiotic_genome_interactions(int genome_length) {
    ...
    abiotic_ints[0] = 0; // start with initially poor fitness I guess
    
    for (int i = 1; i < pow(2,genome_length); i++){
        
      abiotic_ints[i] = 2.0*drand48() - 1.0; // now our range is between -0.5 and 0.5
...
int main(int argc, char **argv) {
    ...
    const double p_kill = 0.002; // probability of microbe death per timestep
    const double prefered_abiotic = 300.0;  // T_ideal

                                    // P(extinction) = a + b|T_ideal - T|
    const double p_ex =  0.000002;  // variable a
    const double p_ex2 = 0.000002;  // variable b
    ...
    const double abiotic_start = 0.0;//190.0;
    const double abiotic_end = 0.0;
...
//    }
    ...
    int t2 = atoi(argv[2]);
    srand48 (t2);
    generator.seed(t2);  //provide seed for randomness in different runs
    ...
    while (end_experiment == 0) {
        ...
        if (timestep_counter >= timestep_length){
            ...
            // Record data here!!!!!
            double extinct_temp = p_ex + p_ex2 * sqrt(pow(abiotic_T - prefered_abiotic, 2.0));

            // Record data here!!!!!
            macro_data << number_gens << " " << species.size() << " "  << abiotic_T << " " << abiotic_env << " " << extinct_temp << " " << total_population << endl;
	    ...
	    **************************************************************************************/
	    ...
	    if (timestep_length > 0) { abiotic_trickle = abiotic_trickle / (1.0*timestep_length); }
 	    ...
 	    double extinct_prob = p_ex + p_ex2 * sqrt(pow(abiotic_T - prefered_abiotic, 2.0));
	    float prob_thing = drand48();
	    if (prob_thing <= extinct_prob) {

		//cout << "timestep: " << number_gens << " random number: " << prob_thing << "  extinct prob: " << extinct_prob << endl;
            
		end_experiment = 1; // exit completely
		//cout << "DONE!";

	    }
	    
	    if (total_population == 0) { cout << "extinction!  ";  end_experiment = 1; }
	    //cout << "timestep: " << number_gens << " random number: " << prob_thing << "  extinct prob: " << extinct_prob << endl;
	    
	}

	    if (end_experiment == 1) { break; }
	  ...
	  if (i_biomass <= starve_thresh) {
	    ...
	    end_experiment = 1;  // so we'll still be told there is a total extinction but the world will go on till the end
	}
	...
	if( i > -1) {
	  ...
	  while ( max_count_eat <= total_count_eat && minimum_count_eat > 0 && total_count_eat > 0 ) {
	    ...
	    if (enough_nutrient == false) { break; }
